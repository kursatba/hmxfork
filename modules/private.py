...
class Private(object):
    ...
    def __init__(
        ...
        eth_provider: Web3,
        eth_signer: Account,
        oracle_middleware: OracleMiddleware,
    ):
    ...
    def get_collaterals(self, sub_account_id: int):
        ...
        for index, collateral in enumerate(COLLATERALS):
            amount = (
                ...
                / 10 ** TOKEN_PROFILE[collateral]["decimals"]
            )
    ...
    def deposit_erc20_collateral(
        self, sub_account_id: int, token_address: str, amount: float
    ):
        """
        ...
        token_instance = load_contract(self.eth_provider, token_address, ERC20_ABI_PATH)

        allowance = token_instance.functions.allowance(
            self.eth_signer.address, CROSS_MARGIN_HANDLER_ADDRESS
        ).call()
        if allowance < amount_wei:
            token_instance.functions.approve(
                CROSS_MARGIN_HANDLER_ADDRESS, amount_wei
        ...
        return cross_margin_handler_instance.functions.depositCollateral(
            sub_account_id, token_address, amount_wei, False
        ).transact({"from": self.eth_signer.address})
    ...
    def deposit_eth_collateral(self, sub_account_id: int, amount: float):
        ...
        return cross_margin_handler_instance.functions.depositCollateral(
            sub_account_id, COLLATERAL_WETH, amount_wei, True
    ...
    def create_market_order(
        ...
        tx = self.__create_order_batch(self.eth_signer.address, sub_account_id, [order])
    ...
    def create_trigger_order(
        ...
        order = {
            ...
            "acceptable_price": Web3.to_wei(
                (
                    self.__add_slippage(trigger_price)
                    if size > 0
                    else self.__sub_slippage(trigger_price)
                ),
                "tether",
            ),
    ...
    def __create_order_batch(self, account: str, sub_account_id: int, orders):
        ...
        return self.limit_trade_handler_instance.functions.batch(
            account, sub_account_id, cmds, datas
        ).transact(
            {
                "from": self.eth_signer.address,
                "value": EXECUTION_FEE * cmds.count(Cmd.CREATE),
            }
        )

    def __prepare_batch_input(self, orders):
        return (
            [order["cmd"] for order in orders],
            [self.__prepare_order_call_data(order) for order in orders],
        )
    ...
    def __prepare_create_order_call_data(
        self,
        market_index: int,
        size_delta: float,
        trigger_price: float,
    ...
    def __prepare_update_order_call_data(
        self,
        order_index: int,
        size_delta: float,
        trigger_price: float,
        ...
        return encode(
            ["uint256", "int256", "uint256", "uint256", "bool", "bool", "address"],
            [
                order_index,
                size_delta,
                trigger_price,
    ...
    def __prepare_cancel_order_call_data(self, order_index: int):
        return encode(["uint256"], [order_index])
    ...
    def __parse_log(self, tx, topic):
        ...
        return self.limit_trade_handler_instance.events[topic]().process_receipt(
            receipt, DISCARD
        )
