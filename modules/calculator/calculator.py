...
class Calculator:
    ...
    @staticmethod
    def get_funding_rate_velocity(
        max_funding_rate: int,
        max_skew_scale_usd: int,
        long_position_size: int,
        short_position_size: int,
        block_timestamp: int,
    ...
    @staticmethod
    def get_funding_rate_velocity_with_out_interval(
        max_funding_rate: int,
        max_skew_scale_usd: int,
        long_position_size: int,
        short_position_size: int,
        ...
        return (
            min(max_funding_rate, ratio)
            if (ratio > 0)
            else max(max_funding_rate * -1, ratio)
        )

    @staticmethod
    def proportional_elapsed_in_day(
        block_timestamp: int, funding_interval: int, last_funding_time: int
    ) -> int:
        elapsed_intervals = (block_timestamp - last_funding_time) // funding_interval
    ...
    def get_borrowing_rate(asset_class_config, asset_class, hlp_tvl):
        return (
            Calculator.get_next_borrowing_rate_without_interval(
                asset_class_config["base_borrowing_rate"],
                asset_class["reserve_value_e30"],
                hlp_tvl,
            )
            // 10**18
        )
    ...
    @staticmethod
    def get_next_borrowing_rate(
        base_borrowing_rate: int,
        reserve_value_e30: int,
        hlp_tvl: int,
        block_timestamp: int,
        last_borrowing_time: int,
        funding_interval: int,
    ):
    ...
    @staticmethod
    def get_next_borrowing_rate_without_interval(
        base_borrowing_rate: int, reserve_value_e30: int, hlp_tvl: int
    ):
        if hlp_tvl == 0:
            return 0
    ...
    @staticmethod
    def get_delta(
        avg_entry_price_e30: int,
        market_price: int,
        is_long: int,
        size: int,
        reserve_value_e30: int,
        last_increase_timestamp: int,
        min_profit_duration: int,
        block_timestamp: int,
    ):
        ...
        price_delta = (
            avg_entry_price_e30 - market_price
            if avg_entry_price_e30 > market_price
            else market_price - avg_entry_price_e30
        )
        delta = size * price_delta // avg_entry_price_e30
        is_profit = (
            market_price > avg_entry_price_e30
            if is_long
            else market_price < avg_entry_price_e30
        )
    ...
    @staticmethod
    def get_next_funding_accrued(
        trading_config, market_config, market, block_timestamp
    ):
        ...
        return (
            ...
            + (
                (market["current_funding_rate"] + next_funding_rate)
                * proportionnal_elapsed_in_day
                // 2
            )
            // 1e18
        )
    ...
    @staticmethod
    def get_funding_fee(
        size: int, current_funding_accrued: int, last_funding_accrued: int
    ):
        funding_rate = current_funding_accrued - last_funding_accrued
        return size * funding_rate // 10**18

    @staticmethod
    def get_borrowing_fee(
        reserved_value: int, sum_borrowing_rate: int, entry_borrowing_rate: int
    ):
