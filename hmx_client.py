...
class Client(object):
    def __init__(
        ...
        pyth_price_service_url=DEFAULT_PYTH_PRICE_SERVICE_URL,
    ):
        self.__eth_provider = Web3(
            Web3.HTTPProvider(rpc_url, request_kwargs={"timeout": 60})
        )
        ...
        dix_oracle = CixOracle(
            DIX_PRICE_ADAPTER_ADDRESS, pyth_oracle, self.__eth_provider
        )
        gm_btc_oracle = GmOracle(
            GM_BTC_PRICE_ADAPTER_ADDRESS,
            [ASSET_BTC, ASSET_BTC, ASSET_USDC],
            pyth_oracle,
            self.__eth_provider,
        )
        gm_eth_oracle = GmOracle(
            GM_ETH_PRICE_ADAPTER_ADDRESS,
            [ASSET_ETH, ASSET_ETH, ASSET_USDC],
            pyth_oracle,
            self.__eth_provider,
        )
        onchain_pricelens_oracle = OnchainPricelensOracle(
            ONCHAIN_PRICELENS_ADDRESS, pyth_oracle, self.__eth_provider
        )
