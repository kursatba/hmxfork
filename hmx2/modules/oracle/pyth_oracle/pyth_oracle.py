...
class PythOracle(object):
    ...
    def get_price(self, asset_id: str):
        params = {"ids[]": self.price_feed_ids[asset_id]}
        try:
            response = requests.get(
                urljoin(self.price_service_endpoint, GET_LATEST_PRICE_FEEDS_ENDPOINT),
                params,
            )
    ...
    def get_multiple_price(self, asset_ids: List[str]):
        params = {
            "ids[]": list(
                map(lambda asset_id: self.price_feed_ids[asset_id], asset_ids)
            )
        }
        try:
            response = requests.get(
            ...
            return_data = list(
                map(
                    lambda price_data: float(price_data["price"]["price"])
                    * 10 ** price_data["price"]["expo"],
                    list(price_datas),
                )
            )
            return return_data
