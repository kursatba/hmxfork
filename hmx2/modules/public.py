...
class Public(object):
    ...
    def __get_all_position(self, account: str, sub_account_list: List[int]):
        ...
        if sub_account_list == []:
            sub_account_list = list(
                map(
                    lambda sub_account_id: self.get_sub_account(
        ...
        position_number_call = list(
            map(
                lambda sub_account_address: self.multicall_instance.create_call(
                    self.perp_storage_instance,
                    "getNumberOfSubAccountPosition",
                    [sub_account_address],
                ),
                sub_account_list,
            )
        ...
        position_calls = list(
            map(
                lambda active_sub_account_address: self.multicall_instance.create_call(
                    self.perp_storage_instance,
                    "getPositionBySubAccount",
        ...
        for sub_account_id in sub_account_list:
            data_pop = data.pop(0)
            (raw_sub_account_position) = decode(
                [
                    "(address,uint256,uint256,uint256,uint256,uint256,int256,int256,int256,uint8)[]"
                ],
                data_pop,
            )
            raw_sub_account_position = [x for y in raw_sub_account_position for x in y]
    ...
    def __multicall_all_market_data(self):
        ...
        market_config_calls = list(
            map(
                lambda market_index: self.multicall_instance.create_call(
        ...
        market_asset_class_map = {
            market_index: market_config_data[market_index]["asset_class"]
            for market_index in ACTIVE_MARKET
        }

        asset_class_list = list(set(market_asset_class_map.values()))

        asset_class_config_calls = list(
            map(
                lambda asset_class: self.multicall_instance.create_call(
        ...
        asset_result = self.multicall_instance.call(asset_call)
        data = asset_result[1]
        ...
        for asset_class in asset_class_list:
            (
                ...
                ["uint256", "uint256", "uint256", "uint256", "uint256"], data.pop(0)
            )
        ...
        for market_index in ACTIVE_MARKET:
            market_info_map[market_index] = {
                ...
                "asset_class_config": asset_class_config_data[
                    market_asset_class_map[market_index]
                ],
    ...
    def __multicall_market_data(self, market_index: int):
        ...
        (
            ...
            sum_borrowing_fee_e30,
            sum_settled_borrowing_fee_e30,
        ) = decode(["uint256", "uint256", "uint256", "uint256", "uint256"], data.pop(0))
        ...
        return {
            ...
            "asset_class_config": {"base_borrowing_rate": base_borrowing_rate},
            "asset_class": {
                "reserve_value_e30": reserve_value_e30,
                "sum_borrowing_rate": sum_borrowing_rate,
                "last_borrowing_time": last_borrowing_time,
    ...
    def get_multiple_price(self, market_indices: List[int] = []):
        ...
        asset_ids = list(
            map(
                lambda market_index: MARKET_PROFILE[market_index]["asset"],
                market_indices,
            )
        )

        price_object = self.oracle_middleware.get_multiple_price(asset_ids)
        ...
        for market_index in market_indices:
            ...
            asset_decimal = MARKET_PROFILE[market_index]["display_decimal"]
            price = round(price_object[asset_id], asset_decimal)
    ...
    def get_all_market_info(self):
        ...
        for market_index in MARKET_PROFILE.keys():
            if market_index not in DELISTED_MARKET:
                ...
                borrowing_rate = Calculator.get_borrowing_rate(
                    current_raw_market_data["asset_class_config"],
                    current_raw_market_data["asset_class"],
                    tvl,
                )
                ...
                market_info[market_index] = {
                    "market": MARKET_PROFILE[market_index]["name"],
                    "price": price,
                    "long_size": current_raw_market_data["market"]["long_position_size"]
                    / 10**30,
                    "short_size": current_raw_market_data["market"][
                        "short_position_size"
                    ]
                    / 10**30,
                    "funding_rate": {
    ...
    def get_all_position_info(self, account: str, sub_account_id: List[int] = []):
        ...
        positions_flat = [x for y in list(positions.values()) for x in y]
        ...
        for position in positions_flat:
            ...
            funding_fee = Calculator.get_funding_fee(
                position["position_size_e30"],
                int(current_funding_accrued),
                position["last_funding_accrued"],
            )
            ...
            borrowing_fee = Calculator.get_borrowing_fee(
                reserved_value=position_reserved_value,
                sum_borrowing_rate=(sum_borrowing_rate + next_borrowing_rate),
                entry_borrowing_rate=entry_borrowing_rate,
            )
    ...
    def get_position_info(self, account: str, sub_account_id: int, market_index: int):
        ...
        block = self.__get_block()
        price = int(
            self.oracle_middleware.get_price(MARKET_PROFILE[market_index]["asset"])
            * 10**30
        )
    ...
    def get_adaptive_fee(self, size_delta: int, market_index: int, is_increase: bool):
        base_fee_bps = 7
        raw_market_config = (
            self.config_storage_instance.functions.getMarketConfigByIndex(
                market_index
            ).call()
        )
        (
        ...
        raw_fee = self.trade_helper_instance.functions.getAdaptiveFeeBps(
            size_delta, market_index, base_fee_bps
        ).call()
